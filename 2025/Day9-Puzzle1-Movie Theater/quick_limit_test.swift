...
func findMaxWithLimit(_ limit: Int) -> Int {
    var maxValid = 0
...
// Try specific limits quickly
let limits = [3_500_000, 4_000_000, 4_500_000, 5_000_000]

for limit in limits {
    cache.removeAll()  // Clear cache between runs
    print("Testing limit \(limit)...")
    let result = findMaxWithLimit(limit)
    print("  Result: \(result)")

    if result == limit {
        print("  ⚠️ Hit the limit - answer might be higher")
    } else if result < limit {
        print("  ✓ Answer found within limit")
        print("\n=== ANSWER: \(result) ===")
        break
    }
}
